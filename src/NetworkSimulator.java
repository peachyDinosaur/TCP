...
public abstract class NetworkSimulator
{
    ...
    protected abstract int  aOutput(Message message);
    ...
    public void runSimulator()
    {
        ...
        while (true)
        {
            ...
            if (traceLevel >= 2)
            {
                ...
                String host = next.getEntity() == 0 ? "A" : next.getEntity() == 1 ? "B" : "INTERNAL PANIC: invalid entity";
                System.out.println("  entity: " + host);
            }
            ...
            if (nSim >= maxMessages)
            {
            	//Special case of last message received from layer 3
            	if(next.getType() == FROMLAYER3)
            	{
            		if (next.getEntity() == A)
                    {
        ...
        if (rand.nextDouble() < corruptProb)
        {
            ...
            if (x < 0.75)
            {
                ...
                if (payload == null) 
				{
                	if (traceLevel >= 2)
                    {
                        System.out.println("toLayer3: sequence number being corrupted");
                    }
				...
				else
				{
					if (traceLevel >= 2)
                    {
                        System.out.println("toLayer3: payload being corrupted");
                    }
            ...
            else
            {
            	if (traceLevel >= 2)
                {
                    System.out.println("toLayer3: acknowledgement number being corrupted");
                }
    ...
    protected void toLayer5(int entity, String msgReceived)
    {
        if (traceLevel > 2)
        {
    ...
    protected void toLayer5(int entity, Message msgReceived)
    {
        if (traceLevel > 2)
        {
            System.out.print("toLayer5: data received:");
